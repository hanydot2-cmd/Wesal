...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  const handleSocialLogin = (providerName: string) => {
    ...
    const fakeEmail = `${providerName.toLowerCase()}_user@wesal.app`;
    const profiles = store.getProfiles();
    let user = profiles.find((p) => p.email === fakeEmail);

    if (!user) {
      user = store.registerUser({
        email: fakeEmail,
        displayName: `مستخدم ${providerName}`,
        gender: 'male',
        isAgeConfirmed: true,
        isTermsAccepted: true
      });
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl max-w-md w-full shadow-2xl border border-rose-100 dark:border-slate-800 relative flex flex-col max-h-[90vh] my-auto overflow-hidden">
        ...
        <div className="space-y-2 mb-6">
          <button
            ...
            className="w-full flex items-center justify-center gap-3 py-2.5 px-4 rounded-xl border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 text-xs font-bold text-slate-700 dark:text-slate-200 transition-colors"
          >
          ...
          <div className="grid grid-cols-2 gap-2">
            <button
              ...
              className="flex items-center justify-center gap-2 py-2 px-3 rounded-xl border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 text-xs font-bold text-slate-700 dark:text-slate-200 transition-colors"
            >
